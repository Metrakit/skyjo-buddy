const iconMap: Record<string, string> = {
  ...
  alertTriangle: '<path d="M21.73 18 13.73 3.99a2 2 0 0 0-3.46 0L2.27 18a2 2 0 0 0 1.73 3h16a2 2 0 0 0 1.73-3"/><line x1="12" x2="12" y1="9" y2="13"/><line x1="12" x2="12.01" y1="17" y2="17"/>'
}

export type IconName = keyof typeof iconMap

/**
 * Create an SVG icon with the given name and optional classes
 */
export function icon(name: IconName, className: string = ''): string {
  const paths = iconMap[name]
  if (!paths) return ''

  return `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="icon ${className}">${paths}</svg>`
}

/**
 * Create an inline icon (for use in buttons, labels, etc.)
 */
export function inlineIcon(name: IconName): string {
  return icon(name, 'inline-icon')
}
